...
public class UserRegisterSteps {
    ...
    public void there_are_the_following_info_in_the_CSV_file(List<String> contacts) throws Throwable {
        ...
        contactToWrite.append(contacts.get(0));
        contactToWrite.append('\n');

        for (int i = 1; i < contacts.size(); i++) {
            String contactDetail = contacts.get(i);
    ...
    public void there_must_be_two_more_contacts_added(List<String> emailList) {
        driver.expectAlert("Batch Contacts Uploaded");
        checkContactsAreCreated(emailList);
        deleteCSVFile();
    }
